...
class VetorTest {
    ...
    @Test
    public void equals() {
        Vetor v = new Vetor(3, 3);
        Vetor vEqual = new Vetor(v.x() + Ponto.tol / 2, v.y() + Ponto.tol / 2);
        assertEquals(v, vEqual);

        Vetor vUnequal = new Vetor(v.x() + Ponto.tol * 2, v.y() + Ponto.tol * 3);
        assertNotEquals(v, vUnequal);
    }
}